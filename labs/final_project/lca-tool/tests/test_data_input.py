...
def sample_data():
    ...
    return pd.DataFrame({
        ...
        'recycling_rate': [0.6, 0.2, 0.2],
        'landfill_rate': [0.3, 0.5, 0.4],
        'incineration_rate': [0.1, 0.3, 0.4],
...
def test_validate_data(sample_data):
    ...
    invalid_data['quantity_kg'] = invalid_data['quantity_kg'].astype(object)  # Fix dtype warning
    invalid_data.loc[0, 'quantity_kg'] = 'invalid'
    assert not data_input.validate_data(invalid_data)

    # Test invalid rates that don’t sum to 1
    invalid_data = sample_data.copy()
